...
export const outbox = (
  ...
  Effect.gen(function* () {
    ...
    const reconcile = (conversation: Conversation, row: IncomingMessage) =>
      Effect.gen(function* () {
        ...
        const matches = yield* sql<SendRow>`SELECT id, state, content, guid, late,
          ...
          FROM send WHERE conversation_id = ${conversation.id} AND kind = 'text'
    ...
    const pending = (conversation: Conversation) =>
      Effect.map(
        sql<SendRow>`SELECT id, state, content, guid, late, notification_pending AS notificationPending,
    ...
    const check = (conversation: Conversation, earlier: SendRow) =>
      Effect.gen(function* () {
        ...
        const state = yield* sql<SendRow>`SELECT id, state, content, guid, late,
          ...
          Effect.gen(function* () {
            ...
            if (earlier) {
              ...
              if (settled.state === "recorded" || settled.state === "uncertain")
                return notSent("an earlier send is still in doubt");
              yield* sql`UPDATE send SET notification_pending = 0 WHERE id = ${settled.id}`;
              if (settled.state === "sent" || settled.state === "delivered") {
                ...
                const result = notSent(
                  `your earlier message "${settled.content}" did go out at ${at}`,
                );
                const now = yield* Clock.currentTimeMillis;
                yield* sql`INSERT INTO send (handle, conversation_id, kind, content, tool_call_id, state, recorded_at, updated_at)
                VALUES (${conversation.handle}, ${conversation.id}, 'text', ${text}, ${callID}, 'not_sent', ${now}, ${now})`;
                return result;
              }
            }
